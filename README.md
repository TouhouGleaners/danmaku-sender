...
> **⚠️ 环境要求**：请确保已安装 **Python 3.12 或更高版本**。
...
首次使用请先前往 **“全局设置”** 标签页：
*   **身份凭证 (Cookie)**：
    *   **获取方法**：浏览器登录 B 站 -> 按 `F12` 打开开发者工具 -> 点击 **“应用” (Application)** 标签 -> 左侧选择 **Cookies** -> 找到 `SESSDATA` 和 `bili_jct` 并复制值。
    *   填写后程序会自动加密存储到系统密钥环中。
...
3.  **配置策略**：
    *   设置随机延迟（如 3.0s - 5.0s）。
    *   建议开启 **“爆发模式”**（例如：每 5 条休息 20 秒）以降低风控概率。
