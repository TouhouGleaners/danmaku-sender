...
3.  处理问题弹幕：
    -   编辑弹幕内容：双击“弹幕内容”列的单元格，可以直接修改弹幕文本。修改后点击回车键或将焦点移出即可保存。
    -   删除弹幕：选择一个或多个有问题的弹幕条目，然后点击“删除选中条目”按钮。这些弹幕将从问题列表中移除，并在应用修改后不会被发送。
    -   应用修改：完成所有处理后，点击“应用所有修改”按钮。所有合法弹幕、以及您已修改或未删除的问题弹幕，将形成新的发送队列。被删除的弹幕将不会被包含在下次发射队列中。
--------------------
【校验规则】
-   内容过长: 弹幕内容不能超过100个字符。
-   包含换行符: 弹幕内容不能包含换行符（\\n 或 \\r）。
