...
def export_danmakus_to_xml(danmakus: list[Danmaku], filepath: str) -> None:
    """
    将弹幕列表导出为纯净的 Bilibili 标准 XML 格式。

    不附加任何失败原因注释，适用于编辑器直接导出。
    """
    root = ET.Element('i')
    root.append(ET.Comment(' Generated by BiliDanmakuSender Editor '))

    sorted_dms = sorted(danmakus, key=lambda x: x.progress)
