...
def create_xml_from_danmakus(danmakus: list, filepath: str):
    """
    将弹幕字典列表转换为B站XML格式并保存到文件。
    
    Args:
        danmakus (list): 弹幕字典列表。每个字典应包含 'progress', 'mode', 'fontsize', 'color', 'msg'。
        filepath (str): 输出XML文件的完整路径。
    """
    root = ET.Element('i')
    ...
    try:
        with open(filepath, 'wb') as f:
            f.write(pretty_xml)
        logger.info(f"✅ 成功将 {len(danmakus)} 条弹幕保存到 '{filepath}'。")
    except Exception as e:
